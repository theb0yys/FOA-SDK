...
{
    TEST(CanonicalInterchangeTypesTests, SemanticIdentifiersUseClosedGrammar)
    {
    ...
    TEST(CanonicalInterchangeTypesTests, VersionAndDigestTokensFailClosed)
    {
        EXPECT_TRUE(IsExactVersionTokenV1("2.7.0"));
        EXPECT_TRUE(IsExactVersionTokenV1("schema-v1"));
        EXPECT_FALSE(IsExactVersionTokenV1(""));
    ...
    TEST(CanonicalInterchangeTypesTests, PackagePathsRejectTraversalAliasesAndPlatformForms)
    {
    ...
    TEST(CanonicalInterchangeTypesTests, ClosedEnumTokensRoundTrip)
    {
        ...
        EXPECT_EQ("tombstone", ToToken(operation));

        ParameterKindV1 parameterKind = ParameterKindV1::String;
        EXPECT_TRUE(TryParseToken("matrix4x4", parameterKind));
        EXPECT_EQ(ParameterKindV1::Matrix4x4, parameterKind);
    }

    TEST(CanonicalInterchangeTypesTests, CanonicalBasisDefaultsAreExact)
    {
        ...
        const CanonicalTemporalBasisV1 temporal;
        EXPECT_EQ("second", temporal.m_timeUnit);
    }
} // namespace TaintedGrailModdingSDK::Interchange
