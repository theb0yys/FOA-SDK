...
class CatalogSchema2ValidatorTests(unittest.TestCase):
    ...
    def _write_valid_contract(self) -> None:
        ...
        self._write(
            ...
            "Schemas 1, 2, 3, 4 and 5 remain load-only migration inputs\n"
        ...
        self._write(
            ...
            "    CurrentCatalogSchemaVersion;\n"
            "}\n"
            "\n    TEST_F(Fixture, CurrentSaveLoadSaveIsByteStable) {}\n"
            "\n    TEST_F(Fixture, CurrentSaveClearLoadAndReplacePreservesCanonicalState) {}\n"
            "\n    TEST_F(Fixture, MalformedCurrentDocumentDoesNotReplacePublishedCatalog) {}\n"
        ...
        self._write(
            ...
            "successful bound replacement followed by `BuildDocument` produces a schema-6 document\n"
            "## Catalog record\n",
        )
        self._write(
        ...
        self._write(
            ...
            "Catalog saves write explicit schema 6\nLegacy O3DE catalog envelopes\n"
            "loaded candidate remains schema 1\n"
    ...
    def test_rejects_stale_roadmap_future_status(self) -> None:
        path = self.repo_root / "ROADMAP.md"
        text = path.read_text(encoding="utf-8").replace(
            "Status: implemented vertical slice, continuing hardening and exact-head host/UI verification.",
            "Status: active development. Core contracts, CatalogDatabase integration",
        )
        path.write_text(text, encoding="utf-8")
        with self.assertRaisesRegex(CatalogSchema2ContractError, "Roadmap population status"):
            validate_catalog_schema2(self.repo_root)
