...
| Install proves the MSI carries the exact reviewed manifest and Start Menu shortcut targets the installed launcher | Package-mode smoke in canonical workflow | `test_lifecycle_smoke_proves_manifest_shortcut_repair_and_uninstall`; staged `FOA-SDK.exe --self-test` | Must pass on Windows package run |
