...
namespace AssetProcessor
{
    ...
    void RCJobListModel::UpdateJobPriority(const QueueElementID& priorityUpdate, int newPriority)
    {
        for (auto foundInQueue = m_jobsInQueueLookup.find(priorityUpdate); foundInQueue != m_jobsInQueueLookup.end(); ++foundInQueue)
        {
            RCJob* job = foundInQueue.value();
            ...
            if (job->GetElementID() != priorityUpdate)
            {
                break;
            }

            if (job->GetPriority() != newPriority)
            {
                UpdateJobPriority(job, newPriority);
            }
        }
    }

    void RCJobListModel::UpdateJobPriority(AssetProcessor::RCJob* rcJob, int newPriority)
    {
        for (int idx = 0; idx < rowCount(); ++idx)
        {
            ...
            if (job == rcJob)
            {
                if (job->GetPriority() != newPriority)
                {
                    ...
                    Q_EMIT dataChanged(index(idx, 0), index(idx, columnCount() - 1)); // need to do this to update the sort model.
                }
                break;
            }
        }
    ...
    void RCJobListModel::PerformHeuristicSearch(QString searchTerm, QString platform, QSet<QueueElementID>& found, AssetProcessor::JobIdEscalationList& escalationList, bool isStatusRequest, int searchRules)
    {
        ...
        QString searchTermWithNoExtension = searchTerm;
        int dotIndex = searchTerm.lastIndexOf('.');
        while (dotIndex != -1)
        {
            searchTermWithNoExtension = searchTerm.mid(0, dotIndex);
            ...
            dotIndex = searchTermWithNoExtension.lastIndexOf('.');
        }
    ...
    bool RCJobListModel::UpdateMissingSourceDependencies(const QString& sourceFileName)
    {
        bool jobsUnblocked = false;
        SourceAssetReference sourceAssetReference(sourceFileName.toUtf8().constData());
        for (RCJob* rcJob : m_jobs)
        {
            jobsUnblocked = jobsUnblocked || rcJob->UpdateMissingDependencies(sourceAssetReference);
        }
        return jobsUnblocked;
    }
}// namespace AssetProcessor
