...
3. In **Pack Manager**, choose **New mod**, enter a name and author, and select **Save mod**. Saving validates and writes the manifest before activating it. A failed save keeps your draft and the previous active mod; correct the reported problem and retry. Saved mods can be selected and reopened from the list. Starting a new draft leaves the previous mod active until the new one is saved. The pane shows **Unsaved changes** when you edit normal or advanced fields. Before **New mod** or **Open selected** replaces those edits, choose **Save**, **Discard**, or **Cancel**. Save continues only after a successful write; a failed save keeps the draft open for correction. Cancel (also Escape or closing the prompt) keeps the draft. Discard replaces it only when the new form or selected mod is ready. Reverting edits to the saved or initial values clears the unsaved warning.
