...
malice; verify the supplied executable checksum and provenance before deciding
to run it.
...
- no Git, Python, CMake, Visual Studio, O3DE source build, or separate MSI.
...
The normal user artifact is the `installer` folder containing
`FOA-SDK-Installer.exe` and `FOA-SDK-Installer.exe.sha256`. Compare the
executable SHA-256 with the sidecar before running it. The checksum verifies the
downloaded executable bytes against the reviewed artifact metadata; current
development artifacts remain unsigned, so it is not a publisher signature.

The reviewed MSI is embedded inside the executable and is not a user-supplied,
user-selected, or user-run artifact. Do not obtain a separate MSI, pair an MSI
with the executable, or run an MSI directly for the normal install flow.

For a portable ZIP recovery artifact, compare its SHA-256 value with the
adjacent `.sha256` file. After extraction, retain these files at the product
root:
...
6. Review the displayed installer fingerprint and external-workspace boundary,
   then select **Apply**.
...
The executable contains the reviewed internal MSI and verifies its captured bytes
before starting Windows Installer. No Python, Git, CMake, Visual Studio, source
checkout, separately installed .NET runtime, or separate MSI handling is
required.

The installed launcher is `FOA-SDK.exe`. It checks the installed manifest,
Editor, and bundled project before starting
`Editor.exe --project-path <installed-project>`. An actionable error asks you to
repair or reinstall when that layout is incomplete.
...
standard fallback for uninstall. A newer reviewed executable containing an
internal MSI with the same Upgrade Code and a newer version replaces the older
product. Do not rename a rebuild to impersonate a new version or install an
older version over a newer one.

Repair restores product-owned files from the exact internal MSI carried by the
reviewed executable. It does not validate or repair external workspaces.
Uninstall removes product-owned files and leaves external workspace data
untouched.

## Maintainer MSI diagnostics

The reviewed MSI may be retained as a separate development artifact for
maintainer/operator diagnosis of Windows Installer behavior. That path is not the
normal user workflow, not the distributed user contract, and not a substitute for
`FOA-SDK-Installer.exe`. Any diagnostic MSI use must remain paired with its exact
reviewed checksum and must never be mixed with an executable from another build.
...
The ZIP and internal MSI contain the same staged manifest payload. The ZIP has no
Windows Installer repair or uninstall registration; remove its extraction
directory manually only after confirming that no workspace was placed inside it.
