...
WorldPlaces, WorldPaths, WorldPathNodes and WorldPathEdges were introduced in catalog schema 5. Current saves use schema 6, which also stores quest authoring. Schemas 1–5 remain supported as validated load inputs. Before the first successful replacement of an older catalog, the writer preserves and verifies a byte-identical sibling backup. A backup or write failure blocks publication. Editors supporting only schemas 1–5 cannot read schema 6; preserve the workspace and restore its earlier catalog backup to return to an earlier version. Keep pack manifests and source/evidence documents alongside the catalog.
