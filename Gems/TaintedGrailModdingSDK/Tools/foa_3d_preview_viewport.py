...
def build_viewport(workspace:Path, ui_render:Path, pane_model:Path, *, output_root:Path|None=None, captured_at:str|None=None, replace:bool=False):
    ...
    if st.get('FunctionCompleteAllowed') is not False or st.get('TypedAuthoringBindingCreated') is not False: raise PreviewViewportError('UI stage escalation')
    rows=ui.get('RenderedEntries') if isinstance(ui.get('RenderedEntries'),list) else []
    ...
    rid='viewport.render.'+prof['ProfileId']+'.'+hashlib.sha256(canon([ui.get('RenderId'),model.get('AssetBrowserModelId'),captured])).hexdigest()[:16]
...
def verify_viewport(path:Path, *, workspace_path:Path|None=None, ui_render_path:Path|None=None, pane_model_path:Path|None=None)->dict[str,Any]:
    ...
    if ui_render_path and d.get('SourceAssetBrowserUiRenderId')!=read_json(ui_render_path).get('RenderId'): raise PreviewViewportError('UI render mismatch')
...
def fixture_inputs(root:Path)->tuple[Path,Path]:
    ...
    rows=[{'SourcePaneEntryId':prod['PaneEntryId'],'ProductAssetIds':prod['ProductAssetIds'],'ProductCachePaths':prod['ProductCachePaths'],'EvidenceRefs':prod['EvidenceRefs'],'CanCreateTypedAuthoringBinding':False,'RequiresExplicitBindingStep':True},{'SourcePaneEntryId':fail['PaneEntryId'],'ProductAssetIds':[],'ProductCachePaths':[],'EvidenceRefs':fail['EvidenceRefs'],'CanCreateTypedAuthoringBinding':False,'RequiresExplicitBindingStep':True,'Issues':fail['Issues']}]
    ui={'SchemaVersion':1,'DocumentKind':UI_KIND,'RenderId':'assetbrowser.render.foa.mono.fixture.synthetic','ProfileId':'foa.mono.fixture','GameVersion':'1.23.401','Branch':'mono','RuntimeTarget':'Mono','SourceAssetBrowserModelId':model['AssetBrowserModelId'],'PreviewStageStatus':{'AssetBrowserPaneUiRendered':True,'TypedAuthoringBindingCreated':False,'TypedSelectorCreated':False,'FunctionCompleteAllowed':False},'RenderedEntries':rows,'OperationalAuthority':auth}
