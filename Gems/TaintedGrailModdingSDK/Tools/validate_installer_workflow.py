...
REQUIRED_FILE_FRAGMENTS = {
    CANONICAL_INSTALLER_WORKFLOW: (
        ...
        "--target TaintedGrailModdingEditorLauncher",
        "FOA-SDK.exe does not match the reviewed installed launcher build.",
        "Build and verify installed FOA-SDK Control Panel",
        "Installer/ControlPanel/Windows/FOAControlPanel.csproj",
        "FOA-SDK-ControlPanel.exe self-test failed before inventory review.",
        "Install hash-pinned CMake runtime into SDK payload",
        '$sdkCmakeRuntime = Join-Path $env:SDK_INSTALL "cmake/runtime"',
        'Copy-Item -Path (Join-Path $toolRoot "*") -Destination $sdkCmakeRuntime -Recurse -Force',
        "Stage reviewed payload, self-test launcher, and create portable ZIP",
        '$stagedLauncher = Join-Path $env:SDK_STAGE "bin/Windows/profile/Default/FOA-SDK.exe"',
        "& $stagedLauncher --self-test",
        "FOA-SDK.exe staged self-contained launcher self-test failed.",
        "FOA-SDK-ControlPanel.exe staged self-test failed.",
        "developer_preview_installer.py inventory",
        "developer_preview_installer.py stage",
    ...
    "Gems/TaintedGrailModdingSDK/Tools/developer_preview_installer.py": (
        ...
        "CMAKE_RUNTIME_CMAKE_PATH",
        "CMAKE_RUNTIME_SHARE_MARKER",
        "PYTHON_CMD_PATH",
        "PYTHON_GET_PYTHON_PATH",
    ...
    "Installer/Packaging/Windows/CMakeLists.txt": (
        ...
        "Windows Installer payload for the prebuilt",
    ...
    "Installer/Launcher/Windows/InstalledEditorLauncher.cpp": (
        ...
        "MaterializedProjectDirectoryName",
        "ExternalDirectoryName",
        "BundledCMakeBinRelativePath",
        "ConfigureBundledRuntimeEnvironment",
        'SetEnvironmentVariableW(L"LY_CMAKE_PATH"',
        "--engine-path",
        "--project-cache-path",
        "--project-user-path",
        "--project-log-path",
    ...
    "Installer/ControlPanel/Windows/app.manifest": (
        'requestedExecutionLevel level="asInvoker"',
    ),
    "Installer/ControlPanel/Windows/Providers/foa.provider.json": (
        '"schema": "foa.sdk.provider_manifest.v1"',
        '"provider_id": "game.foa"',
        '"mode": "user-selected-path"',
        '"machine_scan_allowed": false',
    ...
    "Installer/ControlPanel/Windows/ProviderManifest.cs": (
        ...
        "LoadBundledFoaProvider",
        "ReadLegacyProfile",
        "RequirePathWithoutExistingReparsePoint",
        "AtomicWriteJson",
        "RedactPath",
        "RunSelfTest",
    ),
    "Installer/ControlPanel/Windows/Program.cs": (
        ...
        "ControlPanelForm",
        "SetupManagerCore.Evaluate",
        "SetupManagerCore.Save",
        "SetupManagerCore.ExportSupportReport",
        "Open FOA-SDK",
    ),
    "Installer/Launcher/Windows/FOAInstallerLauncher.csproj": (
        "net8.0-windows",
    ...
    "Installer/Launcher/Windows/Program.cs": (
        ...
        "InstalledEditorLauncher.ValidateAsync",
        "options.ToolWizardOnly",
        "options.SaveToolProfile",
        "ToolSetupProfile.Save",
    ),
    ...
    "Installer/Launcher/Windows/WindowsInstallerRunner.cs": (
        ...
        "CopyLogToEvidenceIfRequested(logPath, evidenceLogPath)",
        'Path.Combine(options.EvidenceRoot, "installer-logs", logFileName)',
        'Arguments = "--self-test"',
        "InstalledEditorValidationResult",
        "CreateDesktopShortcut",
        'Path.Combine(desktop, "FOA-SDK.lnk")',
    ),
    "Installer/Launcher/Windows/InstallerWizardForm.cs": (
        ...
        "InstalledEditorLauncher.ValidateAsync",
        "Open FOA-SDK",
        "Open FOA-SDK Control Panel",
        "Create desktop shortcut",
        "InstalledEditorLauncher.CreateDesktopShortcut",
        "InstalledEditorLauncher.LaunchControlPanel",
        "FOA-SDK is ready",
    ),
    "Installer/Tests/WindowsFunctionalReadiness/Invoke-FoaWindowsFunctionalReadiness.ps1": (
        "foa.sdk.windows_functional_readiness.v1",
        "installer-wizard-smoke",
        "--smoke-test",
        "tool-wizard-ui-smoke",
