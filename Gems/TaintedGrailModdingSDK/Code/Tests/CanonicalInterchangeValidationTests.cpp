...
namespace TaintedGrailModdingSDK::Interchange
{
    ...
    TEST(CanonicalInterchangeValidationTests, StructuralFailureLeavesCallerOutputUntouched)
    {
    ...
    TEST(CanonicalInterchangeValidationTests, ForbiddenAuthorityFieldIsRejected)
    {
        AZStd::string candidate(MinimalDocumentsJson);
    ...
    TEST(CanonicalInterchangeValidationTests, MissingAndCyclicDependenciesAreDeterministic)
    {
        CanonicalInterchangeManifestV1 missing;
        ASSERT_TRUE(ParseCanonicalManifestV1(MinimalAssetJson, missing).IsValid());
        missing.m_payloads[0].m_dependencyIds.push_back({ "payload.missing" });
        const auto missingResult = ValidateCanonicalManifestV1(missing);
        EXPECT_TRUE(ContainsIssue(missingResult, IssueCodeV1::DependencyMissing));

        CanonicalInterchangeManifestV1 cyclic;
        ASSERT_TRUE(ParseCanonicalManifestV1(MinimalAssetJson, cyclic).IsValid());
        ...
        const auto cyclicResult = ValidateCanonicalManifestV1(cyclic);
        EXPECT_TRUE(ContainsIssue(cyclicResult, IssueCodeV1::DependencyCycle));
    }

    TEST(CanonicalInterchangeValidationTests, DeclaredRevisionDriftIsRejected)
    {
        CanonicalInterchangeManifestV1 manifest;
    ...
    TEST(CanonicalInterchangeValidationTests, IssueOrderingAndDuplicateCollapseAreStable)
    {
