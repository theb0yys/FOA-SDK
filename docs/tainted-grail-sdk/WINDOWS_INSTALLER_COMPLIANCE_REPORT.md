...
| Lifecycle smoke checked operation exit codes but did not prove the Start Menu target, shared installed manifest, self-contained launcher layout, or real repair restoration. | The design requires the same manifest payload, a launcher Start Menu entry, staged launcher self-test, and working repair of product-owned files. | Smoke now compares installed/staged manifest hashes, resolves the shortcut target, damages the installed launcher, requires repair to restore its exact hash, and reruns launcher self-test. Commit `7c93011f90eaad693b8fb3314ab9444fb0d137ab`. | `test_lifecycle_smoke_proves_manifest_shortcut_repair_and_uninstall` plus the package-mode Windows smoke itself. |
...
| Installed Control Panel is self-contained, versioned, read-only by default, and included in the canonical payload | `Installer/ControlPanel/Windows`; provider manifest; workflow Control Panel build/self-test; packaging and inventory requirements | `WindowsControlPanelTests`; installer workflow validator; staged Control Panel self-test; functional-readiness profile/report checks | Exact MSI/clean-machine evidence remains tied to the final reviewed package run |
...
| Install proves the MSI carries the exact reviewed manifest and Start Menu shortcuts target the installed launcher and Control Panel | Package-mode smoke in canonical workflow | `test_lifecycle_smoke_proves_manifest_shortcut_repair_and_uninstall`; staged application self-tests; functional-readiness Control Panel checks | Must pass on Windows package run |
...
| Uninstall removes product-owned launcher, Control Panel, manifest, and shortcuts but preserves external workspace data | Package-mode smoke and wizard boundary text | Lifecycle smoke contract test; Windows wizard boundary test | Must pass on Windows package run |
| Installed launcher resolves the self-contained install root, requires root `engine.json`, materializes packaged `External` Gem roots and the packaged project under LocalAppData, opens the bundled Editor with `--engine-path <install-root>`, the materialized project, packaged default level, and writable local app-data launch paths, and has a non-launching self-test | `InstalledEditorLauncher.cpp` | Installer workflow validator and launcher source tests | Full manual Editor UI acceptance remains a separate release gate, as approved |
...
5. require final artifact-set verification and install/manifest/shortcut/staged-launcher-self-test/repair/uninstall smoke to pass;
...
- MSI SHA-256: **pending**
- ZIP SHA-256: **pending**
- Install / launcher self-test / repair / uninstall result: **pending**
