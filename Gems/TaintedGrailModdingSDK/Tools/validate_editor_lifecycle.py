...
def validate_editor_lifecycle(repo_root: Path) -> None:
    if len(PANES) != 30 or len({pane.name for pane in PANES}) != len(PANES):
        raise EditorLifecycleError("The canonical Editor inventory must contain 30 unique pane names")
    ...
    if len(save_keys) != 30 or len(set(save_keys)) != 30:
        raise EditorLifecycleError(
...
def main() -> int:
    ...
    print(
        ...
        "front door, all 30 panes retain build/lifecycle/layout ownership and Hub reachability, "
