...
<#
.SYNOPSIS
Runs Actor and Troop Editor recovery acceptance on a private Windows desktop.
.DESCRIPTION
...
$testScript = Join-Path $PSScriptRoot 'actor_troop_draft_recovery_live_smoke.py'
...
function Invoke-RecoveryCase([string]$Name, [string]$Case, [string]$Reuse = '',
    ...
    if ($Reuse) {
        ...
        if ($fixture.WorkspaceId -ne 'sdkqa.actor-troop-recovery' -or $fixture.RootPath -ne $workspaceRoot) {
        ...
        $fixture = [ordered]@{
            SchemaVersion=1; WorkspaceId=$(if ($Case -eq 'docked-close') {'sdkqa.pack-unsaved'} else {'sdkqa.actor-troop-recovery'}); DisplayName='Synthetic Editor exit acceptance'
    ...
    if ($Corrupt) {
        ...
        if (-not (Test-Inside $copy $OutputRoot) -or -not $copy.EndsWith('.actortroopdrafts.json')) {
    ...
    $editor = [ActorTroopRecoveryTestDesktop]::Start($EditorExecutable, ($quoted -join " "), (Split-Path $EditorExecutable))
    ...
    if ($ForceExit) {
        $passed = $passed -and $intentionalStop -and -not $test.about_to_quit -and
            $test.status -eq 'READY_TO_TERMINATE' -and [ActorTroopRecoveryTestDesktop]::ExitCode() -ne 0
    } else {
        $passed = $passed -and -not $intentionalStop -and [ActorTroopRecoveryTestDesktop]::ExitCode() -eq 0 -and
            $test.status -eq 'PASSED' -and $test.about_to_quit
    }
    $row = [ordered]@{
        status=$(if ($passed) {'PASSED'} else {'FAILED'}); case=$Case; pid=$editor.Id
        exit_code=[ActorTroopRecoveryTestDesktop]::ExitCode(); forced_stop=$forcedStop; about_to_quit=$test.about_to_quit
...
public static class ActorTroopRecoveryTestDesktop {
...
'@
[ActorTroopRecoveryTestDesktop]::Open()
...
try {
    ...
    [ActorTroopRecoveryTestDesktop]::Close()
